...
export type MeroShareSource =
  | "IPO" | "FPO" | "RIGHT" | "AUCTION" | "MARKET" | "BONUS" | "MERGER" | "DEMAT"
...
export async function getMyDetails(token: string): Promise<MeroShareUser> {
...
export async function getMyStocks(token: string): Promise<MeroShareStock[]> {
...
export async function getPurchaseHistory(
  ...
  if (!res.ok) throw new Error(`getPurchaseHistory failed for ${scrip}: ${res.status}`)
  const data = await res.json()
...
export async function fetchAllHoldings(
  token: string
): Promise<{ user: MeroShareUser; lots: EnrichedLot[]; failedScrips: string[] }> {
  const [user, stocks] = await Promise.all([getMyDetails(token), getMyStocks(token)])
  ...
  for (const chunk of chunkArray(stocks, 3)) {
    await Promise.all(
      chunk.map(async (stock) => {
        try {
          const history = await getPurchaseHistory(token, user.demat, stock.scrip)
